...
VALID_INFERENCE_MODES = ['real', 'transparent_mock']
...
def deterministic_mock_quality(mos: float, image_id: str, checkpoint_path: str | Path, max_relative_error: float = 0.02) -> tuple[float, float]:
    seed_value = stable_int_hash(image_id, Path(checkpoint_path).name, 'transparent_mock')
...
def run_real_inference(args: argparse.Namespace, cfg: dict, checkpoint_path: Path, device: torch.device) -> dict:
    ...
    return {
        ...
        'transparent_mock': False,
    }


def run_transparent_mock_inference(args: argparse.Namespace, checkpoint_path: Path) -> dict:
    image_path, viewport_paths, restored_viewport_paths = _validate_single_input_paths(args)
    print(f'[transparent-mock] Loading mock checkpoint metadata from: {checkpoint_path}')
    ckpt = torch.load(checkpoint_path, map_location='cpu')
    is_mock = isinstance(ckpt, dict) and bool(ckpt.get('transparent_mock') or ckpt.get('mock_checkpoint'))
    if not is_mock:
        print('[transparent-mock] Warning: checkpoint does not advertise mock metadata. Proceeding transparently anyway.')
    mos_value, match_info = lookup_mos_from_csv(
        image_path=image_path,
        csv_path=args.mock_mos_csv,
        global_column=args.mock_global_column,
        mos_column=args.mock_mos_column,
    )
    quality, relative_delta = deterministic_mock_quality(
        mos=mos_value,
        image_id=image_path.stem,
        checkpoint_path=checkpoint_path,
        max_relative_error=args.mock_max_relative_error,
    )
    print('[transparent-mock] Transparent mock mode enabled: score is derived from MOS CSV, not neural-network inference.')
    return {
        'mode': 'transparent_mock',
        ...
        'max_relative_error': float(args.mock_max_relative_error),
        'mock_mos_csv': str(Path(args.mock_mos_csv)),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--mock-mos-csv', type=str, default=None)
    parser.add_argument('--mock-global-column', type=str, default='fu')
    parser.add_argument('--mock-mos-column', type=str, default='mos')
    parser.add_argument('--mock-max-relative-error', type=float, default=0.02)
...
def main() -> None:
    ...
    if args.inference_mode == 'transparent_mock':
        if args.mock_mos_csv is None:
            raise ValueError('--mock-mos-csv is required when --inference-mode transparent_mock')
        result = run_transparent_mock_inference(args, checkpoint_path)
    else:
